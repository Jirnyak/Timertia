    ...
    if found1 == 1 and found2 == 1:
        ...
        for d in range(len(generals_list1)):
            if str(update.effective_user.id) == generals_list1[d]:
                generals_list1[d+1] = generals_list[b+1]
...
async def war(update, context):
    ...
    if 1 == 1:
        ...
        if FINAL > 0:
            y = await context.bot.send_message(chat_id=update.effective_chat.id, text=("РФ ОДЕРЖАЛА ПОБЕДУ!" + "\nСилы РФ: " + str(RU_G*100) +  " пехоты " + str(RU_B*10) +  " бронетехники " + str(RU_R) +  " артиллерии " + "\nСилы NATO: " + str(NA_G*100) +  " пехоты " + str(NA_B*10) +  " бронетехники " + str(NA_R) +  " артиллерии "))
            messages.append(y.message_id)
        elif FINAL < 0:
            y = await context.bot.send_message(chat_id=update.effective_chat.id, text=("НАТО ОДЕРЖАЛО ПОБЕДУ!" + "\nСилы РФ: " + str(RU_G*100) +  " пехоты " + str(RU_B*10) +  " бронетехники " + str(RU_R) +  " артиллерии " + "\nСилы NATO: " + str(NA_G*100) +  " пехоты " + str(NA_B*10) +  " бронетехники " + str(NA_R) +  " артиллерии "))
            messages.append(y.message_id)
        else:
            y = await context.bot.send_message(chat_id=update.effective_chat.id, text=("НИЧЬЯ!" + "\nСилы ФР: " + str(RU_G*100) +  " пехоты " + str(RU_B*10) +  " бронетехники " + str(RU_R) +  " артиллерии " + "\nСилы NATO: " + str(NA_G*100) +  " пехоты " + str(NA_B*10) +  " бронетехники " + str(NA_R) +  " артиллерии "))
            messages.append(y.message_id)
